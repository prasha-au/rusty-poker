...
use self::{
  card::{card_to_span, render_card_row},
  seat::{render_seat, Seat, TablePosition},
};

mod card;
mod seat;

fn draw_player<B: Backend>(
  f: &mut Frame<B>,
  table_area: Rect,
  position: u8,
  name: String,
  ...
  let rect = match position {
    0 => Rect {
      x: one_third_x,
      y: table_area.height - rect_h - y_offset,
      width: rect_w,
      height: rect_h,
    },
    7 => Rect {
    ...
    5 => Rect {
      x: table_area.width - rect_w - x_offset,
      y: one_third_y,
      width: rect_w,
      height: rect_h,
    },
...
pub fn draw_table<B: Backend>(f: &mut Frame<B>, game_state: &GameState, table_area: Rect) {
  ...
  render_card_row(
    f,
    Rect {
      x: table_area.width / 2 - 15,
      y: table_area.height / 2,
      width: 30,
      height: 2,
    },
  ...
  let pot_display = Paragraph::new(vec![Spans::from(Span::raw(format!("${}", game_state.total_pot)))])
...
pub fn draw_player_info<B: Backend>(f: &mut Frame<B>, game: &GameState, area: Rect) {
  ...
  let text = vec![Spans::from(text_spans)];
